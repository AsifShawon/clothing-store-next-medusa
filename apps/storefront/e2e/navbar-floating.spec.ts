...
test.describe("London Boy Floating Compact Navbar Suite", () => {
  test("1. Desktop: Header starts expanded at page top and transforms into compact floating navbar on scroll", async ({
    ...
    await page.screenshot({ path: "test-results/screenshots/page-top-desktop.png" })
    ...
    await page.screenshot({ path: "test-results/screenshots/scrolled-compact-header-desktop.png" })

    // Scroll further down over product cards
    await page.evaluate(() => window.scrollTo(0, 1200))
    await page.waitForTimeout(300)
    await expect(header).toHaveAttribute("data-compact", "true")
    await page.screenshot({ path: "test-results/screenshots/compact-header-product-cards.png" })
  ...
  test("2. Shopping actions and navigation links remain interactive in compact mode", async ({
    ...
    // Bag link exists and has accessible name
    const cartLink = header.locator("[data-testid='nav-cart-link']").first()
  ...
  test("3. Mobile: Dedicated compact arrangement with accessible touch targets", async ({
    ...
    await page.screenshot({ path: "test-results/screenshots/mobile-top.png" })
    ...
    await page.screenshot({ path: "test-results/screenshots/mobile-scrolled.png" })
  })
